...
class Grid:
    ...
    def __init__(self, size, width, height, threshold):
        ...
        self.zoom = 4 * size * max(width, height)
        self.theta = 0
        self.phi = 90
    ...
    def drawpath(self, algorithm):
        ...
        if algorithm == "astar":
            ...
            self.dijkstra(self.smallest)
        elif algorithm == "breadth":
            self.breadth_first(self.start)
        path = self.path()
        for tile in path:
            tile.travel(path[-1].distance)
            sleep(0.05)
        print(self.finish.distance)
